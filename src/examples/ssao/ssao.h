...
class SSAO : public IVulkanApp
{
	...
	{
		alignas(16) glm::vec4 kernelSamples[SAMPLE_COUNT];
		alignas(16) glm::mat4 projection;
		alignas(16) glm::vec4 extent;
	};
	...
	void loadModel()
	{
		model = new Model("/resource/models/skull/12140_Skull_v3_L2.obj", VulkanConfig::device, VulkanConfig::physicalDevice, VulkanConfig::graphicsAndComputeQueue);
	...
	void createSwapChainFramebuffers()
	{
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++)
		{
			std::array<VkImageView, 2> attachments = { 
				textures.depthDeferred.imageView,
	...
	void setupUniformBuffers()
	{
		...
		for (size_t i = 0; i < frames; i++)
		{
			...
			const std::string cubeDSName{"uniformBuffers[" + std::string{i} + "].cube"};
			...
			setupDebugObjectName(VK_OBJECT_TYPE_BUFFER, uniformBuffers[i].cube, cubeDSName.c_str());
		};
	}
	...
	void setupDepthDeferred()
	{
		...
		if(vkCreateImage(VulkanConfig::device, &imageInfo, nullptr, &textures.depthDeferred.image))
		{
			throw std::runtime_error("failed to create image!");
		};	

		VkMemoryRequirements memRequirements;
		vkGetImageMemoryRequirements(VulkanConfig::device, textures.depthDeferred.image, &memRequirements);
		...
		if (vkAllocateMemory(VulkanConfig::device, &allocInfo, nullptr, &textures.depthDeferred.imageMemory) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to allocate memory for depth image!");
		};
	
		vkBindImageMemory(VulkanConfig::device, textures.depthDeferred.image, textures.depthDeferred.imageMemory, 0);
		...
		VkImageMemoryBarrier barrier{
			...
			.image = textures.depthDeferred.image	
		};
		...
		if (vkCreateImageView(VulkanConfig::device, &viewInfo, nullptr, &textures.depthDeferred.imageView) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create depth image view!");
		};
		setupDebugObjectName(VK_OBJECT_TYPE_IMAGE, textures.depthDeferred.image, "textures.depthDeferred.image");
		setupDebugObjectName(VK_OBJECT_TYPE_IMAGE_VIEW, textures.depthDeferred.imageView, "textures.depthDeferred.imageView");
	};
	...
	void setupSSAOSampler()
	{
		...
		if (vkCreateSampler(VulkanConfig::device, &samplerInfo, nullptr, &ssaoSampler))
		{
	...
	void setupCubeDescriptorSets()
	{
		...
		std::array<VkDescriptorSetLayoutBinding, 1> setLayoutBindings{uniformLayoutBinding};
		...
		for (size_t i = 0; i < frames; i++)
		{
			...
			const std::string cubeDSName{"descriptorSets[" + std::to_string(i) + "].cube"};
			setupDebugObjectName(VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorSets[i].cube, cubeDSName.c_str());
	...
	void setupQuadDescriptorSets()
	{
		...
		VkDescriptorSetLayoutBinding randomNoiseLayoutBinding
		{
			.binding = 3,
			.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
			.descriptorCount = 1,
		...
		std::array<VkDescriptorSetLayoutBinding, 5> setLayoutBindings{fragmentLayoutBinding, sceneLayoutBinding, albedoLayoutBinding, randomNoiseLayoutBinding, deferredLayoutBinding};
		...
		for (size_t i = 0; i < frames; i++)
		{
			...
			VkDescriptorBufferInfo deferredInfo
			{
				.buffer = uniformBuffers[i].deferred,
				.offset = 0,
				.range = sizeof(DeferredUniform)
			};
			...
			VkDescriptorImageInfo randomNoiseInfo
			{
				.sampler = ssaoSampler,
				.imageView = textures.randomNoise.imageView,
				.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
			};
			...
			descriptorWrites[2].dstSet = descriptorSets[i].quad;
			...
			descriptorWrites[3].pImageInfo = &randomNoiseInfo;
			...
			descriptorWrites[4].pBufferInfo = &deferredInfo;
	...
	void setupCubePipeline()
	{
		...
		shaderStages.clear();
		addShader(SHADER_DIRECTORY + "/ssao/cube/vert.spv", VK_SHADER_STAGE_VERTEX_BIT);	
		addShader(SHADER_DIRECTORY + "/ssao/cube/frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);	
	...
	void setupQuadPipeline()
	{
		...
		shaderStages.clear();
		addShader(SHADER_DIRECTORY + "/ssao/postProcessing/vert.spv", VK_SHADER_STAGE_VERTEX_BIT);	
		addShader(SHADER_DIRECTORY + "/ssao/postProcessing/frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);	
	...
	void setupModelPipeline()
	{
		...
		shaderStages.clear();
		addShader(SHADER_DIRECTORY + "/ssao/object/vert.spv", VK_SHADER_STAGE_VERTEX_BIT);	
		addShader(SHADER_DIRECTORY + "/ssao/object/frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);	
	...
	void updateUniformBuffer(uint32_t currentImage)
	{
		...
		for (int x = -1; x < 2; x++)
		{
			for (int z = -1; z < 2; z++)
			{
				...
				uniformData.model = glm::translate(uniformData.model, glm::vec3(x * 5, -3.0, z * 5));
				uniformData.model = glm::rotate(uniformData.model, glm::radians(270.f), glm::vec3(1., 0., 0.));
				uniformData.model = glm::scale(uniformData.model, glm::vec3(0.1));
		...
		for (unsigned int i = 0; i < 64; ++i)
		{
		   ...
		   sample *= scale;
		   deferredUniform.kernelSamples[i] = sample;
		}

		deferredUniform.projection = glm::perspective(glm::radians(45.f), VulkanConfig::swapChainExtent.width / (float)VulkanConfig::swapChainExtent.height, 0.1f, FAR_PLANE);
		deferredUniform.extent = glm::vec4(VulkanConfig::swapChainExtent.width, VulkanConfig::swapChainExtent.height, 0., 0.);

		// Post Processing Uniform
		memcpy(uniformBuffersMapped[currentImage].deferred, &deferredUniform, sizeof(deferredUniform));

		ObjectUniform cubeUniform;
		cubeUniform.model = glm::mat4(1.);
		cubeUniform.model = glm::scale(cubeUniform.model, glm::vec3(12.));
		cubeUniform.view = camera.getViewMatrix();
		cubeUniform.proj= glm::perspective(glm::radians(45.f), VulkanConfig::swapChainExtent.width/(float) VulkanConfig::swapChainExtent.height, 0.1f, FAR_PLANE);
		cubeUniform.proj[1][1] *= -1;

		memcpy(uniformBuffersMapped[currentImage].cube, &cubeUniform, sizeof(cubeUniform));
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		blitImage(commandBuffer, textures.depth.image, textures.depthDeferred.image);
		...
		vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(SSAO::cubeIndices.size()), 1, 0, 0, 0);
	...
	void blitImage(VkCommandBuffer& commandBuffer, VkImage& srcImage, VkImage& dstImage)
	{
		VkImageMemoryBarrier barrier{};
		...
		blit.srcOffsets[1] = {static_cast<int32_t>(VulkanConfig::swapChainExtent.width),static_cast<int32_t>(VulkanConfig::swapChainExtent.height), 1};
		blit.srcSubresource.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
		blit.srcSubresource.mipLevel = 0;
		blit.srcSubresource.baseArrayLayer = 0;
		blit.srcSubresource.layerCount = 1;
		blit.dstOffsets[0] = {0, 0, 0};
		blit.dstOffsets[1] = {static_cast<int32_t>(VulkanConfig::swapChainExtent.width),static_cast<int32_t>(VulkanConfig::swapChainExtent.height), 1};
	...
	{		
		...
		VkDeviceSize imageSize = 64;
		int mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(4, 4)))) + 1;	
		...
		stbi_image_free(ssaoNoise.data());
		...
		imageInfo.extent.width = static_cast<uint32_t>(4);
		imageInfo.extent.height = static_cast<uint32_t>(4);
		imageInfo.extent.depth = 1;

		if(vkCreateImage(VulkanConfig::device, &imageInfo, nullptr, &textures.randomNoise.image))
		{
			throw std::runtime_error("failed to create image!");
		};	
		
		VkMemoryRequirements memRequirements;
		vkGetImageMemoryRequirements(VulkanConfig::device, textures.randomNoise.image, &memRequirements);
		...
		if (vkAllocateMemory(VulkanConfig::device, &allocInfo, nullptr, &textures.randomNoise.imageMemory) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to allocate image memory!");	
		};
		
		vkBindImageMemory(VulkanConfig::device, textures.randomNoise.image, textures.randomNoise.imageMemory, 0);
		...
		VkImageMemoryBarrier barrier{
			...
			.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
			.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
			.image = textures.randomNoise.image};
		...
		VkBufferImageCopy region{
			...
			.bufferImageHeight = 0,
			.imageOffset = {0,0,0},
			.imageExtent = {static_cast<uint32_t>(4), static_cast<uint32_t>(4), 1}
		};	
		...
		if (vkCreateImageView(VulkanConfig::device, &viewInfo, nullptr, &textures.randomNoise.imageView) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create color image view!");
		};
	}	
